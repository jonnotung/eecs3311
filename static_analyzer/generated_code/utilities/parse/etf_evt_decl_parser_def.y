...
primitive_simple_param_type :
   	  TK_INT  										{ create {ETF_INT_PARAM}      $$                         }
   	| TK_INTEGER  									{ create {ETF_INT_PARAM}      $$                         }
	|          NUMBER TK_UPTO          NUMBER		{ create {ETF_INTERVAL_PARAM} $$.make (     $1,      $3)
													  check_interval_spec ($$) 
													}
	| TK_MINUS NUMBER TK_UPTO          NUMBER	    { create {ETF_INTERVAL_PARAM} $$.make (-1 * $2,      $4) 
													  check_interval_spec ($$) 
													}
	|          NUMBER TK_UPTO TK_MINUS NUMBER		{ create {ETF_INTERVAL_PARAM} $$.make (     $1, -1 * $4) 
													  check_interval_spec ($$) 
													}
	| TK_MINUS NUMBER TK_UPTO TK_MINUS NUMBER       { create {ETF_INTERVAL_PARAM} $$.make (-1 * $2, -1 * $5) 
													  check_interval_spec ($$) 
													}
