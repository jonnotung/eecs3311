note
	description: "[
		Finite list of some elements of type G.
		Sequence positions start with 1.
		]"
	author: "JSO"
	date: "$Date$"
	revision: "$Revision$"
...
feature -- Commands
	...
	insert (v: G; i: INTEGER)
			...
			-- That is, insert(1) on an empty sequence is forbidden.
		require
			valid_position (i)
		local
			j: INTEGER
		...
		do
			...
			loop
				if j = i then
					...
					-- do nothing the proceed with the counting
				end
				imp.forth
				j := j + 1
			end
...
feature -- Quantification
	...
	hold_count (exp: PREDICATE [ANY, TUPLE [PAIR[INTEGER, G]]]): INTEGER
	...
	comprehension alias "|" (exp: PREDICATE [ANY, TUPLE [PAIR[INTEGER, G]]]): like Current
