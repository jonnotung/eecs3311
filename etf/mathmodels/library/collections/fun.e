			...
			no_duplicates:
				across a.lower |..| a.upper as i all
					across a.lower |..| a.upper as j all
						a[i.item] ~ a[j.item] IMPLIES i.item = j.item
					end
				end
		do
			make_empty
...
feature -- Quantifiers
   ...
   hold_count (exp: PREDICATE [ANY, TUPLE [PAIR[G, H]]]): INTEGER
	...
	comprehension alias "|" (exp: PREDICATE [ANY, TUPLE [PAIR[G, H]]]): like Current
