	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			elseif cmd_name ~ "deposit" then 
				 if attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_REAL_ARG} args[2] as amount and then TRUE and then (attached {ETF_TUPLE_ARG} args[3] as date) and then date.value.count = 3 and then (attached {ETF_INT_ARG} date.value[1] as date_d) and then (attached {ETF_INT_ARG} date.value[2] as date_m) and then (attached {ETF_INT_ARG} date.value[3] as date_y) and then 1 <= date_d.value and then date_d.value <= 31 and then 1 <= date_m.value and then date_m.value <= 12 and then 1800 <= date_y.value and then date_y.value <= 2500 then 
					 create {ETF_DEPOSIT} Result.make ("deposit", [id.value , amount.value , [date_d.value, date_m.value, date_y.value]], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			...
			elseif cmd_name ~ "withdraw" then 
				 if attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_REAL_ARG} args[2] as amount and then TRUE and then (attached {ETF_TUPLE_ARG} args[3] as date) and then date.value.count = 3 and then (attached {ETF_INT_ARG} date.value[1] as date_d) and then (attached {ETF_INT_ARG} date.value[2] as date_m) and then (attached {ETF_INT_ARG} date.value[3] as date_y) and then 1 <= date_d.value and then date_d.value <= 31 and then 1 <= date_m.value and then date_m.value <= 12 and then 1800 <= date_y.value and then date_y.value <= 2500 then 
					 create {ETF_WITHDRAW} Result.make ("withdraw", [id.value , amount.value , [date_d.value, date_m.value, date_y.value]], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "transfer" then 
				 if attached {ETF_STR_ARG} args[1] as id1 and then TRUE and then attached {ETF_STR_ARG} args[2] as id2 and then TRUE and then attached {ETF_REAL_ARG} args[3] as amount and then TRUE and then (attached {ETF_TUPLE_ARG} args[4] as date) and then date.value.count = 3 and then (attached {ETF_INT_ARG} date.value[1] as date_d) and then (attached {ETF_INT_ARG} date.value[2] as date_m) and then (attached {ETF_INT_ARG} date.value[3] as date_y) and then 1 <= date_d.value and then date_d.value <= 31 and then 1 <= date_m.value and then date_m.value <= 12 and then 1800 <= date_y.value and then date_y.value <= 2500 then 
					 create {ETF_TRANSFER} Result.make ("transfer", [id1.value , id2.value , amount.value , [date_d.value, date_m.value, date_y.value]], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			else 
		...
		loop
			...
			elseif cmd_name ~ "deposit" then 
				if NOT( ( args.count = 3 ) AND THEN attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_REAL_ARG} args[2] as amount and then TRUE and then (attached {ETF_TUPLE_ARG} args[3] as date) and then date.value.count = 3 and then (attached {ETF_INT_ARG} date.value[1] as date_d) and then (attached {ETF_INT_ARG} date.value[2] as date_m) and then (attached {ETF_INT_ARG} date.value[3] as date_y) and then 1 <= date_d.value and then date_d.value <= 31 and then 1 <= date_m.value and then date_m.value <= 12 and then 1800 <= date_y.value and then date_y.value <= 2500) then 
					...
					Result.append (evt_out_str + " does not conform to declaration " +
							"deposit(id: STRING ; amount: REAL ; date: DATE = TUPLE[d: DAY = 1 .. 31; m: MONTH = 1 .. 12; y: YEAR = 1800 .. 2500])")
				end
			...
			elseif cmd_name ~ "withdraw" then 
				if NOT( ( args.count = 3 ) AND THEN attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_REAL_ARG} args[2] as amount and then TRUE and then (attached {ETF_TUPLE_ARG} args[3] as date) and then date.value.count = 3 and then (attached {ETF_INT_ARG} date.value[1] as date_d) and then (attached {ETF_INT_ARG} date.value[2] as date_m) and then (attached {ETF_INT_ARG} date.value[3] as date_y) and then 1 <= date_d.value and then date_d.value <= 31 and then 1 <= date_m.value and then date_m.value <= 12 and then 1800 <= date_y.value and then date_y.value <= 2500) then 
					...
					Result.append (evt_out_str + " does not conform to declaration " +
							"withdraw(id: STRING ; amount: REAL ; date: DATE = TUPLE[d: DAY = 1 .. 31; m: MONTH = 1 .. 12; y: YEAR = 1800 .. 2500])")
				end
			...
			elseif cmd_name ~ "transfer" then 
				if NOT( ( args.count = 4 ) AND THEN attached {ETF_STR_ARG} args[1] as id1 and then TRUE and then attached {ETF_STR_ARG} args[2] as id2 and then TRUE and then attached {ETF_REAL_ARG} args[3] as amount and then TRUE and then (attached {ETF_TUPLE_ARG} args[4] as date) and then date.value.count = 3 and then (attached {ETF_INT_ARG} date.value[1] as date_d) and then (attached {ETF_INT_ARG} date.value[2] as date_m) and then (attached {ETF_INT_ARG} date.value[3] as date_y) and then 1 <= date_d.value and then date_d.value <= 31 and then 1 <= date_m.value and then date_m.value <= 12 and then 1800 <= date_y.value and then date_y.value <= 2500) then 
					...
					Result.append (evt_out_str + " does not conform to declaration " +
							"transfer(id1: STRING ; id2: STRING ; amount: REAL ; date: DATE = TUPLE[d: DAY = 1 .. 31; m: MONTH = 1 .. 12; y: YEAR = 1800 .. 2500])")
				end
			else
				if NOT Result.is_empty then
					Result.append ("%N")
