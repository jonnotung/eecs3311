...
feature
	...
	make
		do
			add_boolean_case (agent test_single_undo)
...
feature
	...
	test_single_undo: BOOLEAN
		local
			ui: SOLITAIRE_USER_INTERFACE
			bta: BOARD_TEMPLATES_ACCESS
			b: BOARD
		do
			comment ("test_game: single undo")
			create ui.new_cross_game
			Result := ui.game.board ~ bta.templates.cross_board
			check
				Result
			end
			b := ui.game.board.deep_twin
			ui.move_left (3, 4)
			ui.undo
			Result := ui.game.board ~ b and ui.message ~ ui.success
			check
				Result
			end
			ui.move_right (3, 4)
